...
class LossCurve(object):
    def __init__(self):
        self.d_lossCurve = {'steps': [], 'metric': [], 'training_loss': [], 'validation_loss': [], 'validation_acc': []}

    def update(self, step, metric, training_loss, validation_loss=None, validation_acc=None):
    ...
    def plot(self):
        ...
        colors = {'training_loss': 'blue', 'validation_loss': 'orange', 'validation_acc': 'green'}  # Define colors for different metrics
        for metric, color in colors.items():
            indices = [i for i, m in enumerate(self.d_lossCurve['metric']) if m == metric]
...
def save_model(args, model):
    ...
    torch.save({'state_dict': model_to_save.state_dict(), **vars(args)}, model_checkpoint)
...
def valid(args, model, writer, test_loader, global_step):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        with torch.no_grad():
            ...
            preds = torch.argmax(logits, dim=-1) #think this can be commented out, should not be needed since we are not doing classification but regression
...
def main():
    ...
    parser.add_argument("--dataset", default="facemap",
    ...
    parser.add_argument("--num_steps", default=3000, type=int,
                        help="Total number of training epochs to perform.") # plateaus loss at around 1500 steps at current other
    parser.add_argument("--decay_type", choices=["cosine", "linear"], default="linear", #changed from cosine as I believe this is what Yichen did
                        help="How to decay the learning rate.")
    parser.add_argument("--warmup_steps", default=500, type=int,
                        help="Step of training to perform learning rate warmup for.") #was 500
    ...
    parser.add_argument('--gradient_accumulation_steps', type=int, default=1, # tried adjusting this from 1 to 25 to match Yichen
