...
class Block(nn.Module):
    ...
    def load_from(self, weights, n_block):
        ...
        with torch.no_grad():
            ...
            query_bias = np2th(weights[ROOT + "/" + ATTENTION_Q + "/" + "bias"]).view(
                -1
            )
            key_bias = np2th(weights[ROOT + "/" + ATTENTION_K + "/" + "bias"]).view(-1)
            value_bias = np2th(weights[ROOT + "/" + ATTENTION_V + "/" + "bias"]).view(
                -1
            )
            out_bias = np2th(weights[ROOT + "/" + ATTENTION_OUT + "/" + "bias"]).view(
                -1
            )
...
class VisionTransformer(nn.Module):
    def __init__(
        self, config, img_size=224, num_KPs=24, zero_head=False, vis=False
    ):  # defaults to using 24 KPs
        ...
        self.head = Linear(config.hidden_size, num_KPs)

    def forward(self, x, labels=None):
        x, attn_weights = self.transformer(x.float())
